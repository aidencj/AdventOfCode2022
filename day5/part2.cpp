...
int main(){
    ...
    while(getline(input_stream, str)){
        ...
        // I use a temp stack to maintain the order of migrated crates
        stack<char> temp;

        // Pop the crate from the original stack and push it to the temp stack
        for(int i = 0; i < num; i++){
        ...
        // Pop the crate from the temp stack and push it to the destination stack
        for(int i = 0; i < num; i++){
            char crate = temp.top();
            temp.pop();
